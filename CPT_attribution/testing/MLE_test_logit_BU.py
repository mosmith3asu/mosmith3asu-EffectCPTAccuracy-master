...
def main():
    ...
    for i in range(len(xtest)):
        ...
        nLLi.append(MLE.objective(params, R, T, ai_obs))
    fig,axs = plt.subplots(3,1)
    axs[0].plot(xtest,Li)
    axs[1].plot(xtest, LLi)
    axs[2].plot(xtest, nLLi)
    plt.show()
...
def get_CPT_info(fixed):
    ...
    bounds[idx['b']] = (-5.0, 5.0)     #if 'b'       not in fixed else [fixed['b'],fixed['b']]  # reference point
    bounds[idx['gamma']] = (0.2, 2.0)    #if 'gamma'   not in fixed else [fixed['gamma'],fixed['gamma']]  # diminishing return gain
    bounds[idx['lam']] = (0.2, 5.0)    #if 'lam'     not in fixed else [fixed['lam'],fixed['lam']] # loss aversion
    bounds[idx['alpha']] = (0.2, 3.0)   #if 'alpha'   not in fixed else [fixed['alpha'],fixed['alpha']]  # prelec parameter
    bounds[idx['delta']] = (0.2, 0.3)  #if 'delta'   not in fixed else [fixed['delta'],fixed['delta']]  # Probability weight
    bounds[idx['theta']] = (1.0, 10.0) #if 'theta'   not in fixed else [fixed['theta'],fixed['theta']]  # rationality
...
class MLE_Handler():
    ...
    def objective(self, parameters, R,T, ai_obs):
        ...
        # Li = pai[:, ai_obs]
        # LLi = np.log(Li)
        # # LLi = stats.lognorm.cdf(pai[:, ai_obs])
        # negLL = - np.sum(LLi/np.linalg.norm(LLi))
        # # neg_LL = - np.sum(stats.norm.logpdf(y_obs, y_exp, parameters[-1]))
        # return - self.liklihood(parameters,R,T,ai_obs)
