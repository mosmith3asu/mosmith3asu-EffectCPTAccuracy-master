...
class PlotWidgets(object):
    ...
    def add_slider(self,label,vrange,ref=None, vinit=None, height=0.03,step= 0.1,ax=None):
...
def main():
    ...
    widget_caller = [
        lambda axi : Widgets.add_text('Environment Params',ax=axi),
        lambda axi : Widgets.add_slider('Exp. reward center ($\mu_{E}$)', [-30, 30], ref='b',ax=axi),
        lambda axi : Widgets.add_slider(r'Deviation of payoffs ($\sigma_{r})$', [0.01, 10], ref='ell',ax=axi),
        lambda axi : Widgets.add_slider(r'Prob. modification ($\hat{p}_{cert}$)', [-0.5, 0.5],step=0.05, ref='gamma',ax=axi),

        lambda axi: Widgets.add_text('CPT Reward Params',ax=axi),
        lambda axi: Widgets.add_slider('Reference point ($b$)',[-30, 30],ref='b',vinit=CPT_def['b'],ax=axi),
        lambda axi: Widgets.add_slider(r'Diminishing sensitivity ($\gamma$)', [0.01, 1],ref='gamma',vinit=CPT_def['gamma'],ax=axi),
        lambda axi: Widgets.add_slider(r'Loss aversion gain ($\ell$)', [1, 10],ref='ell',vinit=CPT_def['lam'],ax=axi),

        lambda axi: Widgets.add_text('CPT Probability Params ',ax=axi),
        lambda axi: Widgets.add_slider(r'Probability optimism ($\alpha$)', [0.1, 10],ref='alpha',vinit=CPT_def['alpha'],ax=axi),
        lambda axi: Widgets.add_slider(r'Probability sensitivity ($\delta$)', [0.01, 1],ref='delta',vinit=CPT_def['delta'],ax=axi),
        lambda axi: Widgets.add_text('Policy Params',ax=axi),
        lambda axi: Widgets.add_slider(r'Rationality ($\lambda$)', [0.1, 30], ref='lambda',ax=axi)
    ]
    ...
    grid_sz = (2,3*nWidgets)

    axR = plt.subplot2grid(grid_sz, (0*nWidgets, 0), rowspan = nWidgets)
    axP = plt.subplot2grid(grid_sz, (0*nWidgets, 1), rowspan = nWidgets)
    axEn = plt.subplot2grid(grid_sz, (1, 0), rowspan= nWidgets)
    axE0 = plt.subplot2grid(grid_sz, (1, 1), rowspan= nWidgets)
    axEp = plt.subplot2grid(grid_sz, (1, 2), rowspan= nWidgets)
    plot_transforms(axR, axP, CPT_def)

    plt.show()

    widget_axes = []
    for wcall in widget_caller:
